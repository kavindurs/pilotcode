...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch(query) {
        ...
        // Fetch search results via AJAX - using claim search endpoint for claimable businesses only
        fetch(`/api/claim-search?query=${encodeURIComponent(query)}`)
            ...
            .then(data => {
                ...
                // Add business results (only claimable businesses)
                if (data.properties.length > 0) {
                    ...
                    data.properties.slice(0, 5).forEach(property => {
                        ...
                        if (property.profile_picture) {
                            ...
                            placeholder.className = 'w-10 h-10 bg-orange-100 flex items-center justify-center rounded-md text-orange-600 font-semibold';
                        ...
                        status.className = 'text-xs px-2 py-1 rounded-full bg-orange-100 text-orange-600 mt-1 inline-block';
                    ...
                    if (data.properties.length > 5) {
                        ...
                        viewAll.href = `/business-search?search=${encodeURIComponent(query)}`;
                        viewAll.className = 'block px-4 py-3 text-center text-sm text-orange-600 hover:text-orange-800 font-medium border-t border-gray-100 bg-gray-50';
                        viewAll.textContent = `View all claimable businesses`;
    ...
    searchButton.addEventListener('click', function() {
        ...
        if (query) {
            window.location.href = `/business-search?search=${encodeURIComponent(query)}`;
        }
    });
